document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (dynamicImage) {
            ...
            if (isVideo) {
                // Buat elemen video untuk background/header jika source adalah .mp4
                dynamicImage.remove(); // Hapus elemen img lama jika ada
                
                const videoElement = document.createElement('video');
                videoElement.id = 'dynamicImage'; // Pertahankan ID agar selector CSS tetap jalan
                ...
                // Masukkan video ke tempat elemen gambar tadi berada
                // (Asumsi dynamicImage punya parent, kita insert sebelum atau append)
                const parent = dynamicImage.parentNode;
                parent.insertBefore(videoElement, parent.firstChild); // Masukkan video

                // Terapkan logika ukuran pada elemen video baru
                const setMediaSize = () => {
        ...
        async function handleApiRequest(apiUrl, modalRefs, apiName) {
            ...
            try {
                ...
                // Handle VIDEO (MP4 Support)
                    ...
                    video.src = videoUrl;
                    video.controls = true; // Tampilkan tombol play/pause
                    video.autoplay = true;
                    video.loop = true;
